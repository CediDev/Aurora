            ...
            if not message.content.startswith("!") and message.author.id != 1060907451878211624:
                ...
                if last_message_time == None:
                    ...
                    if current_message_time - last_message_time >=30:
                        ...
                        con.commit()
                        if level_id != (new_level := floor(sqrt(user_exp)/5)):
                            cur.execute("UPDATE users SET levelid=? WHERE id=? AND guildid=?", (new_level, message.author.id, guildid))
                            con.commit()
                            embed=discord.Embed(title="Level up!",description=f"Congratulations {message.author.name}, you achieved {level_id} level!")
